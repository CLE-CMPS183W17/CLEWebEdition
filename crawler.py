...
class MyIndexHTMLParser(HTMLParser):
	...
	def handle_starttag(self, tag, attrs):
		...
		if (tag == 'div' and 'id' in attr and attr['id'] == 'content' and self.level == 0):
			...
			courses[attr['href'].split('/')[2]] = {'url' : 'https://courses.soe.ucsc.edu' + attr['href']}
			self.level += 1
...
class MyCourseHTMLParser(HTMLParser):
	...
	def handle_starttag(self, tag, attrs):
		...
		if (tag == 'div' and 'class' in attr and attr['class'] == 'content' and self.level == 0):
			...
			self.level += 1
		elif (tag == 'a' and self.level == 2 and self.inyear and 'href' in attr):
	...
	def handle_data(self, data):
		...
		if (self.level == 1 and self.p):
			...
			if ('Prerequisite(s): ' in data):
				prereqstrdirty = data.split('Prerequisite(s): ')[1]
				prereqstr = fixnames(prereqstrdirty, self.course['name']).split('.')[0]
			...
			if ('Concurrent enrollment in ' in data):
				concurstrdirty = data.split('Concurrent enrollment in ')[1]
				concurstr = fixnames(concurstrdirty, self.course['name']).split(' required.')[0]
